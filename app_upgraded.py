...
<style>
    ...
    .status-light:hover {
        ...
        box-shadow: 0 8px 24px rgba(20, 95, 130, 0.4);
    ...
    .status-indicator {
        ...
        border-radius: 50%;
        background: var(--status-color);
        box-shadow: 0 0 20px var(--status-color), 0 0 40px var(--status-color);
...
# Run UniFi command with error handling
def run_unifi_command(cmd):
    """Execute UniFi CLI command"""
    try:
        ...
        if result.returncode == 0:
            ...
            return f"⚠️ Error: {result.stderr if result.stderr else 'Command failed'}"
...
def render_status_bar(sites):
    ...
    for site in sites:
        ...
        # Truncate long names
        display_name = site_name[:12] + '...' if len(site_name) > 12 else site_name
        
        lights_html += f'''
        <div class="status-light {status_class}">
            <div class="status-indicator"></div>
            <div class="status-label">{display_name}</div>
        </div>
        '''
    ...
    # Create invisible buttons for navigation (Streamlit limitation workaround)
    cols = st.columns(min(len(sites), 10))  # Max 10 per row
    for idx, site in enumerate(sites):
        col_idx = idx % 10
        with cols[col_idx]:
            if st.button(f"→ {site.get('name', 'Unknown')}", key=f"nav_{idx}", help=f"View {site.get('name')} details"):
                st.session_state.current_site = site.get('name')
                st.rerun()
